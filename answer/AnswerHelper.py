...
def query_answer(question, options=None):
    ...
    if options is None:
        options = []
    (index, answer) = DBHelper.get_challenge_answer_from_db(question, options)
    if answer is None:
...
def search_answer(content, options):
    ...
    for option in options:
        x = option.replace("A. ", "").replace("B. ", "").replace(
            "C. ", "").replace("D. ", "").replace("E. ", "")
...
def __get_keywords__(question):
    # www.syiban.com 的搜索不支持以上几种符号以及屏蔽了某些关键词
    splitters = ["，", "。", "（", "）", "？", "、", " ", "　", "共产党"]
...
def __query_from_syiban__(question, keywords, options):
    ...
    for keyword in keywords:
        response = config.SESSION.get(
            "http://www.syiban.com/search/index/init.html?modelid=1&q=" + keyword)
        html = response.html
        ...
        for result in results:
            result_question = result.find('span.title_color', first=True).text.replace(
                'http://www.syiban.com', '')
